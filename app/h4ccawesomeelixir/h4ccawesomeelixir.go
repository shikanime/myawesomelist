...
// ReadContent creates a reader for the README.md file of the h4cc/awesome-elixir repository
...
// DecodeProjects parses projects from the h4cc/awesome-elixir repository
func DecodeProjects(in []byte) ([]Project, error) {
	...
	// Find the Actors section and start parsing from there
	var currentCategory string
	var foundActors bool
	...
	err := ast.Walk(root, func(node ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		case *ast.Heading:
			if n.Level == 2 { // Main category headings are level 2
				...
				// Check if we've reached the Actors section
				if strings.Contains(strings.ToLower(headingText), "actors") {
					foundActors = true
		...
		case *ast.List:
			if foundActors && currentCategory != "" {
	...
	if !foundActors {
		return nil, fmt.Errorf("Actors section not found in the document")
	}
...
// GetProjects parses projects from the h4cc/awesome-elixir repository
