package sorryccawesomejavascript
...
// ReadContent creates a reader for the README.md file of the sorrycc/awesome-javascript repository
...
// DecodeProjects parses projects from the sorrycc/awesome-javascript repository
func DecodeProjects(in []byte) ([]Project, error) {
	...
	// Find the Package Managers section and start parsing from there
	var currentCategory string
	var foundPackageManagers bool
	...
	err := ast.Walk(root, func(node ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		case *ast.Heading:
			if n.Level == 2 { // Main category headings are level 2
				...
				// Check if we've reached the Package Managers section
				if strings.Contains(strings.ToLower(headingText), "package managers") {
					foundPackageManagers = true
					currentCategory = strings.TrimSpace(headingText)
				} else if foundPackageManagers {
		...
		case *ast.List:
			if foundPackageManagers && currentCategory != "" {
	...
	if !foundPackageManagers {
		return nil, fmt.Errorf("Package Managers section not found in the document")
	}
...
// GetProjects parses projects from the sorrycc/awesome-javascript repository
