...
func (db *Database) ListCollections(
	...
	query, params, err := RenderListCollectionsQuery(repos)
	if err != nil {
		return nil, err
	}
	cr, err := db.pg.Query(ctx, query, params...)
	if err != nil {
	...
	var catIDs []uint64
	for i := range cols {
		for j := range cols[i].Categories {
			catIDs = append(catIDs, cols[i].Categories[j].ID)
		}
	}
	if len(catIDs) > 0 {
		...
		if err == nil {
			...
			for i := range cols {
				for j := range cols[i].Categories {
					cols[i].Categories[j].Projects = pm[cols[i].Categories[j].ID]
				}
			}
		}
	}
...
func (db *Database) SearchProjects(
	...
	querySearch, paramsSearch, err := RenderSearchProjectsQuery(repos, embedding, int(limit))
	if err != nil {
		return nil, err
	}
	rows, err := db.pg.Query(ctx, querySearch, paramsSearch...)
	if err != nil {
...
func (db *Database) UpsertProjects(
	...
	for i := range projects {
		b.Queue(
			UpsertProjectQuery,
			projects[i].CategoryID,
			projects[i].RepositoryID,
			projects[i].Name,
	...
	eb := &pgx.Batch{}
	for i := range pes {
		eb.Queue(UpsertProjectEmbeddingQuery, pes[i].ProjectID, pes[i].Embedding)
	}
...
func (db *Database) UpsertProjectMetadata(
	...
	b := &pgx.Batch{}
	for i := range metas {
		b.Queue(UpsertProjectMetadataQuery, metas[i].RepositoryID, metas[i].Readme)
	}
	br := db.pg.SendBatch(ctx, b)
	defer br.Close()
	for range metas {
