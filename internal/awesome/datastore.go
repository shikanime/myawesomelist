		...
		for i, col := range cols {
			...
			for _, cat := range col.Categories {
				...
				for _, p := range cat.Projects {
					c.Projects = append(c.Projects, Project{Repository: RepositoryFromProto(p.Repo), Name: p.Name, Description: p.Description})
				}
				cats = append(cats, c)
			}
...
func (ds *DataStore) UpsertCategories(
	...
	for _, cm := range categories {
		for j := range cm.Projects {
			rms, err := ds.UpsertRepositories(ctx, []*myawesomelistv1.Repository{cm.Projects[j].Repository.ToProto()})
			if err != nil {
